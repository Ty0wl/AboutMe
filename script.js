...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        ...
        setTimeout(() => { setupLegendParticles(); }, 100);
    } catch (err) {
...
function renderTable() {
    ...
    gamesData.forEach(game => {
        ...
        row.dataset.review = game.review;
        row.dataset.perfect = game.perfect || 0;
        
        const reviewData = reviewsData[String(game.id)];
        row.dataset.completion = reviewData?.['game_%'] || 0;

        const perfectBadge = (game.perfect === 1) ? '<div class="perfect-ribbon"></div>' : '';
        
        row.innerHTML = `
            <td><img src="${game.cover}" alt="${game.name}" class="game-cover" loading="lazy"></td>
            <td>${game.year}</td>
            <td>${game.type}</td>
            <td>${game.genre}</td>
            <td>${game.name}</td>
            <td>
                <span class="grade-badge grade-${game.grade.toLowerCase().replace('+', '-plus')} ${game.review === '1' ? 'has-review' : ''} ${game.perfect === 1 ? 'has-perfect' : ''}" 
                data-id="${game.id}" data-review="${game.review}">
                ${game.grade}${perfectBadge}
                </span>
            </td>
        `;
...
function setupGradeClicks() {
    ...
    badges.forEach(badge => {
        ...
        if (hasReview) {
            ...
            badge.addEventListener('click', function(e) { e.stopPropagation(); openReviewModal(this.getAttribute('data-id')); });
            badge.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openReviewModal(badge.getAttribute('data-id')); } });
        } else {
            badge.style.cursor = 'default';
            badge.addEventListener('click', function(e) { e.stopPropagation(); showReviewToast(); });
        }
    });
}
...
function setupLegendParticles() {
    ...
    const gradeParticleMap = {
        'grade-a': 'resources/gfx/particles/particle_a.png', 'grade-b': 'resources/gfx/particles/particle_b.png',
        'grade-c': 'resources/gfx/particles/particle_c.png', 'grade-d': 'resources/gfx/particles/particle_d.png',
        'grade-e': 'resources/gfx/particles/particle_e.png', 'grade-f': 'resources/gfx/particles/particle_f.png',
    };
    badges.forEach(badge => {
        badge.style.cursor = 'pointer';
        badge.addEventListener('click', (e) => {
            ...
            for (const [cls, path] of Object.entries(gradeParticleMap)) {
                if (badge.classList.contains(cls)) { particlePath = path; break; }
            }
...
function spawnParticles(originX, originY, imagePath) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        particle.src = `${imagePath}?t=${Date.now()}-${i}`; particle.alt = '';
        particle.style.position = 'fixed'; particle.style.left = originX + 'px'; particle.style.top = originY + 'px';
        particle.style.width = '18px'; particle.style.height = '18px';
        particle.style.pointerEvents = 'none'; particle.style.zIndex = '999999';
        particle.style.opacity = '1'; particle.style.transform = 'translate(-50%, -50%) scale(1)';
        particle.style.imageRendering = 'pixelated';
        document.body.appendChild(particle);
        const angle = (Math.PI * 2 * i) / count + Math.random() * 0.5;
        const distance = 70 + Math.random() * 60;
        const tx = Math.cos(angle) * distance, ty = Math.sin(angle) * distance - 30;
...
function renderRatingIcons(value, type = 'star', max = 5) {
    ...
    for (let i = 1; i <= max; i++) {
        ...
        html += `<img src="${src}" alt="${i <= value ? 'filled' : 'empty'}" class="rating-icon" width="24" height="24" draggable="false" loading="lazy">`;
    }
    html += '</div>';
    return html;
}
...
async function openReviewModal(gameId) {
    ...
    const reviewData = { ...review, ...translatedReview };
    
    if (reviewData) {
        document.getElementById('modal-title').innerHTML = `<div class="review-header"><span>${reviewData.title || `Game #${gameId}`}</span></div>`;
        ...
        } else if (screenshotsContainer) screenshotsContainer.style.display = 'none';
...
function showReviewToast() { const toast = document.getElementById('review-toast'); toast.classList.add('active'); clearTimeout(toastTimeout); toastTimeout = setTimeout(() => toast.classList.remove('active'), 5000); }
function closeReviewToast() { document.getElementById('review-toast').classList.remove('active'); clearTimeout(toastTimeout); }
...
function setupModalEvents() {
    ...
    const modal = document.getElementById('review-modal');
    if (modal) modal.addEventListener('click', (e) => { if (e.target === modal) closeReviewModal(); });
    document.addEventListener('keydown', (e) => { if (e.key === 'Escape') { closeReviewModal(); closeReviewToast(); } });
}
...
function initFilters() {
    ...
    const filterPerfect = document.getElementById('filter-perfect');
    if (filterReviews) filterReviews.addEventListener('change', applyFilters);
    if (filterPerfect) filterPerfect.addEventListener('change', applyFilters);
}
...
async function setLanguage(langCode) {
    ...
    try {
        ...
        elements.forEach(el => { el.classList.add('i18n-loading'); el.classList.remove('i18n-loaded'); });
        const response = await fetch(path);
        if (!response.ok) throw new Error(`HTTP ${response.status}: Файл не найден`);
        const dictionary = await response.json();
        console.log('JSON успешно загружен');
        if (dictionary.page_title) document.title = dictionary.page_title;
        let replacedCount = 0;
        ...
        document.querySelectorAll('[data-i18n]').forEach(el => { el.classList.remove('i18n-loading'); el.classList.add('i18n-loaded'); });
...
function initSettings() {
    ...
    document.querySelectorAll('[data-i18n]').forEach(el => el.classList.add('i18n-loading'));
    const settingsBtn = document.getElementById('settings-btn');
    const settingsPanel = document.getElementById('settings-panel');
    const overlay = document.getElementById('settings-overlay');
    const closeBtn = document.getElementById('settings-close');
    const langSelector = document.getElementById('language-selector');
    if (settingsBtn) settingsBtn.addEventListener('click', (e) => { e.preventDefault(); settingsPanel.classList.add('active'); overlay.classList.add('active'); });
    if (closeBtn) closeBtn.addEventListener('click', closePanel);
    if (overlay) overlay.addEventListener('click', closePanel);
    function closePanel() { settingsPanel.classList.remove('active'); overlay.classList.remove('active'); }
...
function renderScreenshots(screenshots, container) {
    ...
    container.innerHTML = '';
    if (screenshots.length === 0) { container.innerHTML = '<div class="screenshots-loading">Скриншоты не найдены</div>'; container.style.display = 'block'; return; }
    screenshots.forEach((screenshotUrl, index) => {
        const img = document.createElement('img');
        img.src = screenshotUrl; img.alt = `Screenshot ${index + 1}`; img.className = 'screenshot-item'; img.loading = 'lazy'; img.draggable = false;
        img.addEventListener('click', () => window.open(screenshotUrl, '_blank'));
        img.onload = () => console.log(`Загружен скриншот #${index + 1}`);
        img.onerror = () => console.error(`Ошибка загрузки: ${screenshotUrl}`);
        container.appendChild(img);
    });
    container.style.display = 'flex';
    setTimeout(() => initCustomScrollbar(), 150);
}
...
function initCustomScrollbar() {
    ...
    let isDragging = false, thumbRaf = null, scrollTarget = 0, scrollCurrent = 0, scrollRaf = null;
    
    function smoothScrollLoop() {
        scrollCurrent += (scrollTarget - scrollCurrent) * 0.12;
        container.scrollLeft = scrollCurrent;
        updateThumbPosition();
        if (Math.abs(scrollTarget - scrollCurrent) > 0.5) scrollRaf = requestAnimationFrame(smoothScrollLoop);
        else { scrollRaf = null; scrollCurrent = scrollTarget; }
    }
    function updateThumbPosition() {
        if (thumbRaf) return;
        thumbRaf = requestAnimationFrame(() => {
            const scrollWidth = container.scrollWidth, clientWidth = container.clientWidth, scrollLeft = container.scrollLeft;
            if (scrollWidth <= clientWidth) { scrollbar.style.opacity = '0'; scrollbar.style.pointerEvents = 'none'; thumbRaf = null; return; }
            scrollbar.style.opacity = '1'; scrollbar.style.pointerEvents = 'auto';
    ...
    container.addEventListener('wheel', (e) => {
        ...
        if (!scrollRaf) { scrollCurrent = container.scrollLeft; scrollRaf = requestAnimationFrame(smoothScrollLoop); }
    }, { passive: false });
    container.addEventListener('scroll', updateThumbPosition, { passive: true });
    const onDragStart = (e) => { isDragging = true; document.body.style.cursor = 'grabbing'; e.preventDefault(); if (scrollRaf) { cancelAnimationFrame(scrollRaf); scrollRaf = null; } };
    const onDragMove = (e) => {
        ...
        scrollTarget = percent * maxScroll; scrollCurrent = scrollTarget;
        container.scrollLeft = scrollCurrent; updateThumbPosition();
    };
    const onDragEnd = () => { if (isDragging) { isDragging = false; document.body.style.cursor = ''; } };
    const onTrackClick = (e) => {
        ...
        const percent = mouseX / rect.width;
        const maxScroll = container.scrollWidth - container.clientWidth;
        scrollTarget = percent * maxScroll; scrollCurrent = container.scrollLeft;
